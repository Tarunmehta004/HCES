...
class hh_quintile_exp:
    """
    Calculates item-wise average expenditure for each quintile.
    """

    #Initialization
    def __init__ (self, current_level_path: str, level_15_path: str, quintile_path: str, ic_combined: list, hh_size_str: Literal['F', 'C', 'D'], ic_7days = [], ic_30days = [], ic_365days = [],cons_column = 'cons_total_value'):
        """
        Args:
            ...
            quintile_path (str): Path of the .pkl file containing quintile mapping for HHs. 
        ...
        #Loading hh quintile pickle file
        self.quintile_df = pd.read_pickle(os.path.normpath(quintile_path))
        print("\n"*2 + "Quintile file loaded successfully".center(100,"="))
        display(self.quintile_df.head())
        ...
        if cons_column in self.level_df:
            if all([col in self.level_df for col in ['item_code', 'hhid',cons_column]]):
                ...
                print(f"Total number of distinct hhid = {self.quintile_df.shape[0]}")
        ...
        ## Merging scaled pivot table with hh_quintile
        self.merged_df = pd.merge(self.quintile_df, self.hh_scaled_df,
                                  how = 'outer',
                                  left_index=True, 
                                  right_index=True) 
        print("\n"*2 + "Quintile and scaled pivot file merged successfully.".center(100,"="))
        print(f"Check all hhids in merged file are covered? \n{self.merged_df.shape[0] == self.quintile_df.shape[0]}")
    ...
    def wt_avg (self):
        ...
        mult_sum = df.groupby(by = 'hh_quintile', observed = False)['multiplier'].apply('sum')
        sum_df = df.groupby(by = 'hh_quintile', observed = False)[self.ic_list_df + ['total expenditure']].apply('sum')
...
r"""if __name__ == "__main__":
    ...
    quintile_path = r"G:\.shortcut-targets-by-id\1NprIdwv7vnADEhIOU_jU6YsVBoFvikqW\Coal Research\HCES 2022-23\Python implementation\Codes\HH quintile level expenditure\hh_quintile.pkl"
    ...
    level_10 = hh_quintile_exp( current_level_path= level_path, 
